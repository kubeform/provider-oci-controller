...
// AnomalyDetectionDataAssetReconciler reconciles a AnomalyDetectionDataAsset object
type AnomalyDetectionDataAssetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectiondataassets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectiondataassets/status,verbs=get;update;patch

func (r *AnomalyDetectionDataAssetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anomalydetectiondataasset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnomalyDetectionDataAsset")
...
func (r *AnomalyDetectionDataAssetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &aiv1alpha1.AnomalyDetectionDataAsset{}); err != nil {
			klog.Error(err, "unable to set up auditor", aiv1alpha1.AnomalyDetectionDataAsset{}.APIVersion, aiv1alpha1.AnomalyDetectionDataAsset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&aiv1alpha1.AnomalyDetectionDataAsset{}).
...
func (r *AnomalyDetectionDataAssetReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnomalyDetectionDataAsset")
			return nil
		}
		...
		resourceList := &aiv1alpha1.AnomalyDetectionDataAssetList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnomalyDetectionDataAsset")
			return nil
		}
