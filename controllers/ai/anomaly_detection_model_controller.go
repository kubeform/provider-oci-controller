...
// AnomalyDetectionModelReconciler reconciles a AnomalyDetectionModel object
type AnomalyDetectionModelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectionmodels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectionmodels/status,verbs=get;update;patch

func (r *AnomalyDetectionModelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anomalydetectionmodel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnomalyDetectionModel")
...
func (r *AnomalyDetectionModelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &aiv1alpha1.AnomalyDetectionModel{}); err != nil {
			klog.Error(err, "unable to set up auditor", aiv1alpha1.AnomalyDetectionModel{}.APIVersion, aiv1alpha1.AnomalyDetectionModel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&aiv1alpha1.AnomalyDetectionModel{}).
