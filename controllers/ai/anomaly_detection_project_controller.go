...
// AnomalyDetectionProjectReconciler reconciles a AnomalyDetectionProject object
type AnomalyDetectionProjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectionprojects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ai.oci.kubeform.com,resources=anomalydetectionprojects/status,verbs=get;update;patch

func (r *AnomalyDetectionProjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anomalydetectionproject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnomalyDetectionProject")
...
func (r *AnomalyDetectionProjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &aiv1alpha1.AnomalyDetectionProject{}); err != nil {
			klog.Error(err, "unable to set up auditor", aiv1alpha1.AnomalyDetectionProject{}.APIVersion, aiv1alpha1.AnomalyDetectionProject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&aiv1alpha1.AnomalyDetectionProject{}).
