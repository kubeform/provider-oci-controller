...
// +kubebuilder:rbac:groups=analytics.oci.kubeform.com,resources=analyticsinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=analytics.oci.kubeform.com,resources=analyticsinstances/status,verbs=get;update;patch

func (r *AnalyticsInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsInstance")
...
func (r *AnalyticsInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &analyticsv1alpha1.AnalyticsInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", analyticsv1alpha1.AnalyticsInstance{}.APIVersion, analyticsv1alpha1.AnalyticsInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&analyticsv1alpha1.AnalyticsInstance{}).
...
func (r *AnalyticsInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsInstance")
			return nil
		}
		...
		resourceList := &analyticsv1alpha1.AnalyticsInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsInstance")
			return nil
		}
