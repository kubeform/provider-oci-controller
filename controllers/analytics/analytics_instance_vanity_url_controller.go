...
// AnalyticsInstanceVanityURLReconciler reconciles a AnalyticsInstanceVanityURL object
type AnalyticsInstanceVanityURLReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=analytics.oci.kubeform.com,resources=analyticsinstancevanityurls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=analytics.oci.kubeform.com,resources=analyticsinstancevanityurls/status,verbs=get;update;patch

func (r *AnalyticsInstanceVanityURLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsinstancevanityurl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsInstanceVanityURL")
...
func (r *AnalyticsInstanceVanityURLReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &analyticsv1alpha1.AnalyticsInstanceVanityURL{}); err != nil {
			klog.Error(err, "unable to set up auditor", analyticsv1alpha1.AnalyticsInstanceVanityURL{}.APIVersion, analyticsv1alpha1.AnalyticsInstanceVanityURL{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&analyticsv1alpha1.AnalyticsInstanceVanityURL{}).
...
func (r *AnalyticsInstanceVanityURLReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AnalyticsInstanceVanityURL")
			return nil
		}
		...
		resourceList := &analyticsv1alpha1.AnalyticsInstanceVanityURLList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AnalyticsInstanceVanityURL")
			return nil
		}
