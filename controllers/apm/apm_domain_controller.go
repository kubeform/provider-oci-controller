...
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=apmdomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=apmdomains/status,verbs=get;update;patch

func (r *ApmDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apmdomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApmDomain")
...
func (r *ApmDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apmv1alpha1.ApmDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", apmv1alpha1.ApmDomain{}.APIVersion, apmv1alpha1.ApmDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apmv1alpha1.ApmDomain{}).
