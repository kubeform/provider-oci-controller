...
// SyntheticsMonitorReconciler reconciles a SyntheticsMonitor object
type SyntheticsMonitorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=syntheticsmonitors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=syntheticsmonitors/status,verbs=get;update;patch

func (r *SyntheticsMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("syntheticsmonitor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SyntheticsMonitor")
...
func (r *SyntheticsMonitorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apmv1alpha1.SyntheticsMonitor{}); err != nil {
			klog.Error(err, "unable to set up auditor", apmv1alpha1.SyntheticsMonitor{}.APIVersion, apmv1alpha1.SyntheticsMonitor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apmv1alpha1.SyntheticsMonitor{}).
...
func (r *SyntheticsMonitorReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SyntheticsMonitor")
			return nil
		}
		...
		resourceList := &apmv1alpha1.SyntheticsMonitorList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SyntheticsMonitor")
			return nil
		}
