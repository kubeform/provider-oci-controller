...
// SyntheticsScriptReconciler reconciles a SyntheticsScript object
type SyntheticsScriptReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=syntheticsscripts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apm.oci.kubeform.com,resources=syntheticsscripts/status,verbs=get;update;patch

func (r *SyntheticsScriptReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("syntheticsscript", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SyntheticsScript")
...
func (r *SyntheticsScriptReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apmv1alpha1.SyntheticsScript{}); err != nil {
			klog.Error(err, "unable to set up auditor", apmv1alpha1.SyntheticsScript{}.APIVersion, apmv1alpha1.SyntheticsScript{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apmv1alpha1.SyntheticsScript{}).
