...
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerconfigurations/status,verbs=get;update;patch

func (r *ContainerConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerConfiguration")
...
func (r *ContainerConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &artifactsv1alpha1.ContainerConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", artifactsv1alpha1.ContainerConfiguration{}.APIVersion, artifactsv1alpha1.ContainerConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&artifactsv1alpha1.ContainerConfiguration{}).
