...
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerimagesignatures,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerimagesignatures/status,verbs=get;update;patch

func (r *ContainerImageSignatureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerimagesignature", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerImageSignature")
...
func (r *ContainerImageSignatureReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &artifactsv1alpha1.ContainerImageSignature{}); err != nil {
			klog.Error(err, "unable to set up auditor", artifactsv1alpha1.ContainerImageSignature{}.APIVersion, artifactsv1alpha1.ContainerImageSignature{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&artifactsv1alpha1.ContainerImageSignature{}).
