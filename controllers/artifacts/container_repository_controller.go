...
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerrepositories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=containerrepositories/status,verbs=get;update;patch

func (r *ContainerRepositoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerrepository", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerRepository")
...
func (r *ContainerRepositoryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &artifactsv1alpha1.ContainerRepository{}); err != nil {
			klog.Error(err, "unable to set up auditor", artifactsv1alpha1.ContainerRepository{}.APIVersion, artifactsv1alpha1.ContainerRepository{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&artifactsv1alpha1.ContainerRepository{}).
