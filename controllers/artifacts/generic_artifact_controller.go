...
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=genericartifacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=artifacts.oci.kubeform.com,resources=genericartifacts/status,verbs=get;update;patch

func (r *GenericArtifactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("genericartifact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GenericArtifact")
...
func (r *GenericArtifactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &artifactsv1alpha1.GenericArtifact{}); err != nil {
			klog.Error(err, "unable to set up auditor", artifactsv1alpha1.GenericArtifact{}.APIVersion, artifactsv1alpha1.GenericArtifact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&artifactsv1alpha1.GenericArtifact{}).
