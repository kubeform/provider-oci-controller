...
// +kubebuilder:rbac:groups=bastion.oci.kubeform.com,resources=bastions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bastion.oci.kubeform.com,resources=bastions/status,verbs=get;update;patch
...
func (r *BastionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Bastion")
...
func (r *BastionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bastionv1alpha1.Bastion{}); err != nil {
			klog.Error(err, "unable to set up auditor", bastionv1alpha1.Bastion{}.APIVersion, bastionv1alpha1.Bastion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bastionv1alpha1.Bastion{}).
