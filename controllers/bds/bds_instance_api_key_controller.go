...
// BdsInstanceAPIKeyReconciler reconciles a BdsInstanceAPIKey object
type BdsInstanceAPIKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bds.oci.kubeform.com,resources=bdsinstanceapikeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bds.oci.kubeform.com,resources=bdsinstanceapikeys/status,verbs=get;update;patch

func (r *BdsInstanceAPIKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bdsinstanceapikey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BdsInstanceAPIKey")
...
func (r *BdsInstanceAPIKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bdsv1alpha1.BdsInstanceAPIKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", bdsv1alpha1.BdsInstanceAPIKey{}.APIVersion, bdsv1alpha1.BdsInstanceAPIKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bdsv1alpha1.BdsInstanceAPIKey{}).
...
func (r *BdsInstanceAPIKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for BdsInstanceAPIKey")
			return nil
		}
		...
		resourceList := &bdsv1alpha1.BdsInstanceAPIKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BdsInstanceAPIKey")
			return nil
		}
