...
// +kubebuilder:rbac:groups=bds.oci.kubeform.com,resources=bdsinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bds.oci.kubeform.com,resources=bdsinstances/status,verbs=get;update;patch

func (r *BdsInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bdsinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BdsInstance")
...
func (r *BdsInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bdsv1alpha1.BdsInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", bdsv1alpha1.BdsInstance{}.APIVersion, bdsv1alpha1.BdsInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bdsv1alpha1.BdsInstance{}).
...
func (r *BdsInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for BdsInstance")
			return nil
		}
		...
		resourceList := &bdsv1alpha1.BdsInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BdsInstance")
			return nil
		}
