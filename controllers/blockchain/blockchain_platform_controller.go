...
// +kubebuilder:rbac:groups=blockchain.oci.kubeform.com,resources=blockchainplatforms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=blockchain.oci.kubeform.com,resources=blockchainplatforms/status,verbs=get;update;patch

func (r *BlockchainPlatformReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("blockchainplatform", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BlockchainPlatform")
...
func (r *BlockchainPlatformReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &blockchainv1alpha1.BlockchainPlatform{}); err != nil {
			klog.Error(err, "unable to set up auditor", blockchainv1alpha1.BlockchainPlatform{}.APIVersion, blockchainv1alpha1.BlockchainPlatform{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&blockchainv1alpha1.BlockchainPlatform{}).
...
func (r *BlockchainPlatformReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for BlockchainPlatform")
			return nil
		}
		...
		resourceList := &blockchainv1alpha1.BlockchainPlatformList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BlockchainPlatform")
			return nil
		}
