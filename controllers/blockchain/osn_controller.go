...
// +kubebuilder:rbac:groups=blockchain.oci.kubeform.com,resources=osns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=blockchain.oci.kubeform.com,resources=osns/status,verbs=get;update;patch

func (r *OsnReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("osn", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Osn")
...
func (r *OsnReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &blockchainv1alpha1.Osn{}); err != nil {
			klog.Error(err, "unable to set up auditor", blockchainv1alpha1.Osn{}.APIVersion, blockchainv1alpha1.Osn{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&blockchainv1alpha1.Osn{}).
