...
// +kubebuilder:rbac:groups=budget.oci.kubeform.com,resources=alertrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=budget.oci.kubeform.com,resources=alertrules/status,verbs=get;update;patch

func (r *AlertRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertrule", req.NamespacedName)
...
func (r *AlertRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &budgetv1alpha1.AlertRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", budgetv1alpha1.AlertRule{}.APIVersion, budgetv1alpha1.AlertRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&budgetv1alpha1.AlertRule{}).
