...
// ManagementCaBundleReconciler reconciles a ManagementCaBundle object
type ManagementCaBundleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=certificates.oci.kubeform.com,resources=managementcabundles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=certificates.oci.kubeform.com,resources=managementcabundles/status,verbs=get;update;patch

func (r *ManagementCaBundleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managementcabundle", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagementCaBundle")
...
func (r *ManagementCaBundleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &certificatesv1alpha1.ManagementCaBundle{}); err != nil {
			klog.Error(err, "unable to set up auditor", certificatesv1alpha1.ManagementCaBundle{}.APIVersion, certificatesv1alpha1.ManagementCaBundle{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&certificatesv1alpha1.ManagementCaBundle{}).
