...
// GuardDetectorRecipeReconciler reconciles a GuardDetectorRecipe object
type GuardDetectorRecipeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guarddetectorrecipes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guarddetectorrecipes/status,verbs=get;update;patch

func (r *GuardDetectorRecipeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("guarddetectorrecipe", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GuardDetectorRecipe")
...
func (r *GuardDetectorRecipeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.GuardDetectorRecipe{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.GuardDetectorRecipe{}.APIVersion, cloudv1alpha1.GuardDetectorRecipe{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.GuardDetectorRecipe{}).
