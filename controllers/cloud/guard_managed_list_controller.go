...
// GuardManagedListReconciler reconciles a GuardManagedList object
type GuardManagedListReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardmanagedlists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardmanagedlists/status,verbs=get;update;patch

func (r *GuardManagedListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("guardmanagedlist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GuardManagedList")
...
func (r *GuardManagedListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.GuardManagedList{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.GuardManagedList{}.APIVersion, cloudv1alpha1.GuardManagedList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.GuardManagedList{}).
