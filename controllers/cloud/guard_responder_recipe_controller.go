...
// GuardResponderRecipeReconciler reconciles a GuardResponderRecipe object
type GuardResponderRecipeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardresponderrecipes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardresponderrecipes/status,verbs=get;update;patch

func (r *GuardResponderRecipeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("guardresponderrecipe", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GuardResponderRecipe")
...
func (r *GuardResponderRecipeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.GuardResponderRecipe{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.GuardResponderRecipe{}.APIVersion, cloudv1alpha1.GuardResponderRecipe{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.GuardResponderRecipe{}).
