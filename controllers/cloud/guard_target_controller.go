...
// GuardTargetReconciler reconciles a GuardTarget object
type GuardTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardtargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.oci.kubeform.com,resources=guardtargets/status,verbs=get;update;patch

func (r *GuardTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("guardtarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GuardTarget")
...
func (r *GuardTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.GuardTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.GuardTarget{}.APIVersion, cloudv1alpha1.GuardTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.GuardTarget{}).
