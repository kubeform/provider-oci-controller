...
// DefaultDHCPOptionsReconciler reconciles a DefaultDHCPOptions object
type DefaultDHCPOptionsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultdhcpoptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultdhcpoptions/status,verbs=get;update;patch

func (r *DefaultDHCPOptionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultdhcpoptions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultDHCPOptions")
...
func (r *DefaultDHCPOptionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.DefaultDHCPOptions{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.DefaultDHCPOptions{}.APIVersion, corev1alpha1.DefaultDHCPOptions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.DefaultDHCPOptions{}).
