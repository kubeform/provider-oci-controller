...
// DefaultRouteTableReconciler reconciles a DefaultRouteTable object
type DefaultRouteTableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultroutetables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultroutetables/status,verbs=get;update;patch

func (r *DefaultRouteTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultroutetable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultRouteTable")
...
func (r *DefaultRouteTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.DefaultRouteTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.DefaultRouteTable{}.APIVersion, corev1alpha1.DefaultRouteTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.DefaultRouteTable{}).
