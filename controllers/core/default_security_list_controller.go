...
// DefaultSecurityListReconciler reconciles a DefaultSecurityList object
type DefaultSecurityListReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultsecuritylists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=defaultsecuritylists/status,verbs=get;update;patch

func (r *DefaultSecurityListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultsecuritylist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultSecurityList")
...
func (r *DefaultSecurityListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.DefaultSecurityList{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.DefaultSecurityList{}.APIVersion, corev1alpha1.DefaultSecurityList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.DefaultSecurityList{}).
