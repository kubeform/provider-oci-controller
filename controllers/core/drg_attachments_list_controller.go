...
// DrgAttachmentsListReconciler reconciles a DrgAttachmentsList object
type DrgAttachmentsListReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=drgattachmentslists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=drgattachmentslists/status,verbs=get;update;patch

func (r *DrgAttachmentsListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("drgattachmentslist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DrgAttachmentsList")
...
func (r *DrgAttachmentsListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.DrgAttachmentsList{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.DrgAttachmentsList{}.APIVersion, corev1alpha1.DrgAttachmentsList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.DrgAttachmentsList{}).
