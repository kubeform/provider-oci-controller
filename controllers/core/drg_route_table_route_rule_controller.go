...
// DrgRouteTableRouteRuleReconciler reconciles a DrgRouteTableRouteRule object
type DrgRouteTableRouteRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=drgroutetablerouterules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=drgroutetablerouterules/status,verbs=get;update;patch

func (r *DrgRouteTableRouteRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("drgroutetablerouterule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DrgRouteTableRouteRule")
...
func (r *DrgRouteTableRouteRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.DrgRouteTableRouteRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.DrgRouteTableRouteRule{}.APIVersion, corev1alpha1.DrgRouteTableRouteRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.DrgRouteTableRouteRule{}).
