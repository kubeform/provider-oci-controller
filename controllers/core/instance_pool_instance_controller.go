...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=instancepoolinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=instancepoolinstances/status,verbs=get;update;patch

func (r *InstancePoolInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("instancepoolinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstancePoolInstance")
...
func (r *InstancePoolInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.InstancePoolInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.InstancePoolInstance{}.APIVersion, corev1alpha1.InstancePoolInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.InstancePoolInstance{}).
