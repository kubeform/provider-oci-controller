...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=internetgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=internetgateways/status,verbs=get;update;patch

func (r *InternetGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("internetgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InternetGateway")
...
func (r *InternetGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.InternetGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.InternetGateway{}.APIVersion, corev1alpha1.InternetGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.InternetGateway{}).
