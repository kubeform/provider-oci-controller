...
// IpsecConnectionTunnelManagementReconciler reconciles a IpsecConnectionTunnelManagement object
type IpsecConnectionTunnelManagementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=ipsecconnectiontunnelmanagements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=ipsecconnectiontunnelmanagements/status,verbs=get;update;patch

func (r *IpsecConnectionTunnelManagementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipsecconnectiontunnelmanagement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpsecConnectionTunnelManagement")
...
func (r *IpsecConnectionTunnelManagementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.IpsecConnectionTunnelManagement{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.IpsecConnectionTunnelManagement{}.APIVersion, corev1alpha1.IpsecConnectionTunnelManagement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.IpsecConnectionTunnelManagement{}).
