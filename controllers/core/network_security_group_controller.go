...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=networksecuritygroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=networksecuritygroups/status,verbs=get;update;patch

func (r *NetworkSecurityGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networksecuritygroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkSecurityGroup")
...
func (r *NetworkSecurityGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.NetworkSecurityGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.NetworkSecurityGroup{}.APIVersion, corev1alpha1.NetworkSecurityGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.NetworkSecurityGroup{}).
