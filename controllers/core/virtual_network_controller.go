...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=virtualnetworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=virtualnetworks/status,verbs=get;update;patch
...
func (r *VirtualNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualNetwork")
...
func (r *VirtualNetworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.VirtualNetwork{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.VirtualNetwork{}.APIVersion, corev1alpha1.VirtualNetwork{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.VirtualNetwork{}).
