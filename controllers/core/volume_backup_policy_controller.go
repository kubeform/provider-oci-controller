...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=volumebackuppolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=volumebackuppolicies/status,verbs=get;update;patch

func (r *VolumeBackupPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("volumebackuppolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VolumeBackupPolicy")
...
func (r *VolumeBackupPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.VolumeBackupPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.VolumeBackupPolicy{}.APIVersion, corev1alpha1.VolumeBackupPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.VolumeBackupPolicy{}).
