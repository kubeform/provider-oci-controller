...
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=volumegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.oci.kubeform.com,resources=volumegroups/status,verbs=get;update;patch

func (r *VolumeGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("volumegroup", req.NamespacedName)
...
func (r *VolumeGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &corev1alpha1.VolumeGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", corev1alpha1.VolumeGroup{}.APIVersion, corev1alpha1.VolumeGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.VolumeGroup{}).
