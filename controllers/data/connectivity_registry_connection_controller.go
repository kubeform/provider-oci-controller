...
// ConnectivityRegistryConnectionReconciler reconciles a ConnectivityRegistryConnection object
type ConnectivityRegistryConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistryconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistryconnections/status,verbs=get;update;patch

func (r *ConnectivityRegistryConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectivityregistryconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectivityRegistryConnection")
...
func (r *ConnectivityRegistryConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.ConnectivityRegistryConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.ConnectivityRegistryConnection{}.APIVersion, datav1alpha1.ConnectivityRegistryConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.ConnectivityRegistryConnection{}).
