...
// ConnectivityRegistryReconciler reconciles a ConnectivityRegistry object
type ConnectivityRegistryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistries/status,verbs=get;update;patch

func (r *ConnectivityRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectivityregistry", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectivityRegistry")
...
func (r *ConnectivityRegistryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.ConnectivityRegistry{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.ConnectivityRegistry{}.APIVersion, datav1alpha1.ConnectivityRegistry{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.ConnectivityRegistry{}).
