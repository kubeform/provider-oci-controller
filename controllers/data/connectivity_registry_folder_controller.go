...
// ConnectivityRegistryFolderReconciler reconciles a ConnectivityRegistryFolder object
type ConnectivityRegistryFolderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistryfolders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=connectivityregistryfolders/status,verbs=get;update;patch

func (r *ConnectivityRegistryFolderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectivityregistryfolder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectivityRegistryFolder")
...
func (r *ConnectivityRegistryFolderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.ConnectivityRegistryFolder{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.ConnectivityRegistryFolder{}.APIVersion, datav1alpha1.ConnectivityRegistryFolder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.ConnectivityRegistryFolder{}).
