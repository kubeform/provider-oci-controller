...
// SafeSecurityAssessmentReconciler reconciles a SafeSecurityAssessment object
type SafeSecurityAssessmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safesecurityassessments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safesecurityassessments/status,verbs=get;update;patch

func (r *SafeSecurityAssessmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("safesecurityassessment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SafeSecurityAssessment")
...
func (r *SafeSecurityAssessmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.SafeSecurityAssessment{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.SafeSecurityAssessment{}.APIVersion, datav1alpha1.SafeSecurityAssessment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.SafeSecurityAssessment{}).
