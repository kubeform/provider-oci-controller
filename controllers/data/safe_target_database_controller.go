...
// SafeTargetDatabaseReconciler reconciles a SafeTargetDatabase object
type SafeTargetDatabaseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safetargetdatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safetargetdatabases/status,verbs=get;update;patch

func (r *SafeTargetDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("safetargetdatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SafeTargetDatabase")
...
func (r *SafeTargetDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.SafeTargetDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.SafeTargetDatabase{}.APIVersion, datav1alpha1.SafeTargetDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.SafeTargetDatabase{}).
...
func (r *SafeTargetDatabaseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SafeTargetDatabase")
			return nil
		}
		...
		resourceList := &datav1alpha1.SafeTargetDatabaseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SafeTargetDatabase")
			return nil
		}
