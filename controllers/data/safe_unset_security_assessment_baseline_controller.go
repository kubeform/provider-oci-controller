...
// SafeUnsetSecurityAssessmentBaselineReconciler reconciles a SafeUnsetSecurityAssessmentBaseline object
type SafeUnsetSecurityAssessmentBaselineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safeunsetsecurityassessmentbaselines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safeunsetsecurityassessmentbaselines/status,verbs=get;update;patch

func (r *SafeUnsetSecurityAssessmentBaselineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("safeunsetsecurityassessmentbaseline", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SafeUnsetSecurityAssessmentBaseline")
...
func (r *SafeUnsetSecurityAssessmentBaselineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.SafeUnsetSecurityAssessmentBaseline{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.SafeUnsetSecurityAssessmentBaseline{}.APIVersion, datav1alpha1.SafeUnsetSecurityAssessmentBaseline{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.SafeUnsetSecurityAssessmentBaseline{}).
