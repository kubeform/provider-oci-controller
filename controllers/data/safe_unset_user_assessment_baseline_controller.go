...
// SafeUnsetUserAssessmentBaselineReconciler reconciles a SafeUnsetUserAssessmentBaseline object
type SafeUnsetUserAssessmentBaselineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safeunsetuserassessmentbaselines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.oci.kubeform.com,resources=safeunsetuserassessmentbaselines/status,verbs=get;update;patch

func (r *SafeUnsetUserAssessmentBaselineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("safeunsetuserassessmentbaseline", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SafeUnsetUserAssessmentBaseline")
...
func (r *SafeUnsetUserAssessmentBaselineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.SafeUnsetUserAssessmentBaseline{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.SafeUnsetUserAssessmentBaseline{}.APIVersion, datav1alpha1.SafeUnsetUserAssessmentBaseline{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.SafeUnsetUserAssessmentBaseline{}).
