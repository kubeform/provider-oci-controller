...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomouscontainerdatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomouscontainerdatabases/status,verbs=get;update;patch

func (r *AutonomousContainerDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autonomouscontainerdatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutonomousContainerDatabase")
...
func (r *AutonomousContainerDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.AutonomousContainerDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.AutonomousContainerDatabase{}.APIVersion, databasev1alpha1.AutonomousContainerDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.AutonomousContainerDatabase{}).
...
func (r *AutonomousContainerDatabaseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AutonomousContainerDatabase")
			return nil
		}
		...
		resourceList := &databasev1alpha1.AutonomousContainerDatabaseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AutonomousContainerDatabase")
			return nil
		}
