...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomousdatabasebackups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomousdatabasebackups/status,verbs=get;update;patch

func (r *AutonomousDatabaseBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autonomousdatabasebackup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutonomousDatabaseBackup")
...
func (r *AutonomousDatabaseBackupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.AutonomousDatabaseBackup{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.AutonomousDatabaseBackup{}.APIVersion, databasev1alpha1.AutonomousDatabaseBackup{}.Kind)
			return err
		}
	}
