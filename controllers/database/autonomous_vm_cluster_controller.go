...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomousvmclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=autonomousvmclusters/status,verbs=get;update;patch

func (r *AutonomousVmClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autonomousvmcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutonomousVmCluster")
...
func (r *AutonomousVmClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.AutonomousVmCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.AutonomousVmCluster{}.APIVersion, databasev1alpha1.AutonomousVmCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.AutonomousVmCluster{}).
