...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=backupdestinations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=backupdestinations/status,verbs=get;update;patch

func (r *BackupDestinationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backupdestination", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackupDestination")
...
func (r *BackupDestinationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.BackupDestination{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.BackupDestination{}.APIVersion, databasev1alpha1.BackupDestination{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.BackupDestination{}).
