...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=cloudvmclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=cloudvmclusters/status,verbs=get;update;patch

func (r *CloudVmClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudvmcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudVmCluster")
...
func (r *CloudVmClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.CloudVmCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.CloudVmCluster{}.APIVersion, databasev1alpha1.CloudVmCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.CloudVmCluster{}).
