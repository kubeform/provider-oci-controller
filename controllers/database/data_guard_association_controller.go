...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dataguardassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dataguardassociations/status,verbs=get;update;patch

func (r *DataGuardAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dataguardassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataGuardAssociation")
...
func (r *DataGuardAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.DataGuardAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.DataGuardAssociation{}.APIVersion, databasev1alpha1.DataGuardAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.DataGuardAssociation{}).
...
func (r *DataGuardAssociationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DataGuardAssociation")
			return nil
		}
		...
		resourceList := &databasev1alpha1.DataGuardAssociationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DataGuardAssociation")
			return nil
		}
