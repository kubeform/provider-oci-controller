...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dbhomes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dbhomes/status,verbs=get;update;patch

func (r *DbHomeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dbhome", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DbHome")
...
func (r *DbHomeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.DbHome{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.DbHome{}.APIVersion, databasev1alpha1.DbHome{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.DbHome{}).
...
func (r *DbHomeReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DbHome")
			return nil
		}
		...
		resourceList := &databasev1alpha1.DbHomeList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DbHome")
			return nil
		}
