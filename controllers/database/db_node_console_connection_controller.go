...
// DbNodeConsoleConnectionReconciler reconciles a DbNodeConsoleConnection object
type DbNodeConsoleConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dbnodeconsoleconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=dbnodeconsoleconnections/status,verbs=get;update;patch

func (r *DbNodeConsoleConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dbnodeconsoleconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DbNodeConsoleConnection")
...
func (r *DbNodeConsoleConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.DbNodeConsoleConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.DbNodeConsoleConnection{}.APIVersion, databasev1alpha1.DbNodeConsoleConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.DbNodeConsoleConnection{}).
