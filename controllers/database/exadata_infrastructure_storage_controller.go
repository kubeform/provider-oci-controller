...
// ExadataInfrastructureStorageReconciler reconciles a ExadataInfrastructureStorage object
type ExadataInfrastructureStorageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=exadatainfrastructurestorages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=exadatainfrastructurestorages/status,verbs=get;update;patch

func (r *ExadataInfrastructureStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("exadatainfrastructurestorage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExadataInfrastructureStorage")
...
func (r *ExadataInfrastructureStorageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ExadataInfrastructureStorage{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ExadataInfrastructureStorage{}.APIVersion, databasev1alpha1.ExadataInfrastructureStorage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ExadataInfrastructureStorage{}).
