...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externalcontainerdatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externalcontainerdatabases/status,verbs=get;update;patch

func (r *ExternalContainerDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externalcontainerdatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalContainerDatabase")
...
func (r *ExternalContainerDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ExternalContainerDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ExternalContainerDatabase{}.APIVersion, databasev1alpha1.ExternalContainerDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ExternalContainerDatabase{}).
