...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externaldatabaseconnectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externaldatabaseconnectors/status,verbs=get;update;patch

func (r *ExternalDatabaseConnectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externaldatabaseconnector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalDatabaseConnector")
...
func (r *ExternalDatabaseConnectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ExternalDatabaseConnector{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ExternalDatabaseConnector{}.APIVersion, databasev1alpha1.ExternalDatabaseConnector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ExternalDatabaseConnector{}).
...
func (r *ExternalDatabaseConnectorReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ExternalDatabaseConnector")
			return nil
		}
		...
		resourceList := &databasev1alpha1.ExternalDatabaseConnectorList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ExternalDatabaseConnector")
			return nil
		}
