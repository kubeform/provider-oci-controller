...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externalnoncontainerdatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=externalnoncontainerdatabases/status,verbs=get;update;patch

func (r *ExternalNonContainerDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externalnoncontainerdatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalNonContainerDatabase")
...
func (r *ExternalNonContainerDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ExternalNonContainerDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ExternalNonContainerDatabase{}.APIVersion, databasev1alpha1.ExternalNonContainerDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ExternalNonContainerDatabase{}).
