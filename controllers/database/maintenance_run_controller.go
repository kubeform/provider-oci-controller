...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=maintenanceruns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=maintenanceruns/status,verbs=get;update;patch

func (r *MaintenanceRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("maintenancerun", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MaintenanceRun")
...
func (r *MaintenanceRunReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.MaintenanceRun{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.MaintenanceRun{}.APIVersion, databasev1alpha1.MaintenanceRun{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.MaintenanceRun{}).
