...
// ManagementManagedDatabaseGroupReconciler reconciles a ManagementManagedDatabaseGroup object
type ManagementManagedDatabaseGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=managementmanageddatabasegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=managementmanageddatabasegroups/status,verbs=get;update;patch

func (r *ManagementManagedDatabaseGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managementmanageddatabasegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagementManagedDatabaseGroup")
...
func (r *ManagementManagedDatabaseGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ManagementManagedDatabaseGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ManagementManagedDatabaseGroup{}.APIVersion, databasev1alpha1.ManagementManagedDatabaseGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ManagementManagedDatabaseGroup{}).
