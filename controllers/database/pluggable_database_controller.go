...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabases/status,verbs=get;update;patch

func (r *PluggableDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pluggabledatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PluggableDatabase")
...
func (r *PluggableDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.PluggableDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.PluggableDatabase{}.APIVersion, databasev1alpha1.PluggableDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.PluggableDatabase{}).
...
func (r *PluggableDatabaseReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PluggableDatabase")
			return nil
		}
		...
		resourceList := &databasev1alpha1.PluggableDatabaseList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PluggableDatabase")
			return nil
		}
