...
// PluggableDatabasesLocalCloneReconciler reconciles a PluggableDatabasesLocalClone object
type PluggableDatabasesLocalCloneReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabaseslocalclones,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabaseslocalclones/status,verbs=get;update;patch

func (r *PluggableDatabasesLocalCloneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pluggabledatabaseslocalclone", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PluggableDatabasesLocalClone")
...
func (r *PluggableDatabasesLocalCloneReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.PluggableDatabasesLocalClone{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.PluggableDatabasesLocalClone{}.APIVersion, databasev1alpha1.PluggableDatabasesLocalClone{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.PluggableDatabasesLocalClone{}).
...
func (r *PluggableDatabasesLocalCloneReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PluggableDatabasesLocalClone")
			return nil
		}
		...
		resourceList := &databasev1alpha1.PluggableDatabasesLocalCloneList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PluggableDatabasesLocalClone")
			return nil
		}
