...
// PluggableDatabasesRemoteCloneReconciler reconciles a PluggableDatabasesRemoteClone object
type PluggableDatabasesRemoteCloneReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabasesremoteclones,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=pluggabledatabasesremoteclones/status,verbs=get;update;patch

func (r *PluggableDatabasesRemoteCloneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pluggabledatabasesremoteclone", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PluggableDatabasesRemoteClone")
...
func (r *PluggableDatabasesRemoteCloneReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.PluggableDatabasesRemoteClone{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.PluggableDatabasesRemoteClone{}.APIVersion, databasev1alpha1.PluggableDatabasesRemoteClone{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.PluggableDatabasesRemoteClone{}).
...
func (r *PluggableDatabasesRemoteCloneReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PluggableDatabasesRemoteClone")
			return nil
		}
		...
		resourceList := &databasev1alpha1.PluggableDatabasesRemoteCloneList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PluggableDatabasesRemoteClone")
			return nil
		}
