...
// ToolsDatabaseToolsConnectionReconciler reconciles a ToolsDatabaseToolsConnection object
type ToolsDatabaseToolsConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=toolsdatabasetoolsconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=toolsdatabasetoolsconnections/status,verbs=get;update;patch

func (r *ToolsDatabaseToolsConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("toolsdatabasetoolsconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ToolsDatabaseToolsConnection")
...
func (r *ToolsDatabaseToolsConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ToolsDatabaseToolsConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ToolsDatabaseToolsConnection{}.APIVersion, databasev1alpha1.ToolsDatabaseToolsConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ToolsDatabaseToolsConnection{}).
