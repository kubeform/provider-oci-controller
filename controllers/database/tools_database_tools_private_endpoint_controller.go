...
// ToolsDatabaseToolsPrivateEndpointReconciler reconciles a ToolsDatabaseToolsPrivateEndpoint object
type ToolsDatabaseToolsPrivateEndpointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=toolsdatabasetoolsprivateendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=toolsdatabasetoolsprivateendpoints/status,verbs=get;update;patch

func (r *ToolsDatabaseToolsPrivateEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("toolsdatabasetoolsprivateendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ToolsDatabaseToolsPrivateEndpoint")
...
func (r *ToolsDatabaseToolsPrivateEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ToolsDatabaseToolsPrivateEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ToolsDatabaseToolsPrivateEndpoint{}.APIVersion, databasev1alpha1.ToolsDatabaseToolsPrivateEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ToolsDatabaseToolsPrivateEndpoint{}).
