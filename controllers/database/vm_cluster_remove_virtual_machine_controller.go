...
// VmClusterRemoveVirtualMachineReconciler reconciles a VmClusterRemoveVirtualMachine object
type VmClusterRemoveVirtualMachineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=vmclusterremovevirtualmachines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.oci.kubeform.com,resources=vmclusterremovevirtualmachines/status,verbs=get;update;patch

func (r *VmClusterRemoveVirtualMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vmclusterremovevirtualmachine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VmClusterRemoveVirtualMachine")
...
func (r *VmClusterRemoveVirtualMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.VmClusterRemoveVirtualMachine{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.VmClusterRemoveVirtualMachine{}.APIVersion, databasev1alpha1.VmClusterRemoveVirtualMachine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.VmClusterRemoveVirtualMachine{}).
