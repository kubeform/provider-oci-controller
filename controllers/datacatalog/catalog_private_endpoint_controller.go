...
// +kubebuilder:rbac:groups=datacatalog.oci.kubeform.com,resources=catalogprivateendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datacatalog.oci.kubeform.com,resources=catalogprivateendpoints/status,verbs=get;update;patch

func (r *CatalogPrivateEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogprivateendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogPrivateEndpoint")
...
func (r *CatalogPrivateEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datacatalogv1alpha1.CatalogPrivateEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", datacatalogv1alpha1.CatalogPrivateEndpoint{}.APIVersion, datacatalogv1alpha1.CatalogPrivateEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datacatalogv1alpha1.CatalogPrivateEndpoint{}).
