...
// +kubebuilder:rbac:groups=datacatalog.oci.kubeform.com,resources=metastores,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datacatalog.oci.kubeform.com,resources=metastores/status,verbs=get;update;patch

func (r *MetastoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metastore", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Metastore")
...
func (r *MetastoreReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datacatalogv1alpha1.Metastore{}); err != nil {
			klog.Error(err, "unable to set up auditor", datacatalogv1alpha1.Metastore{}.APIVersion, datacatalogv1alpha1.Metastore{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datacatalogv1alpha1.Metastore{}).
