...
// InvokeRunReconciler reconciles a InvokeRun object
type InvokeRunReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dataflow.oci.kubeform.com,resources=invokeruns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataflow.oci.kubeform.com,resources=invokeruns/status,verbs=get;update;patch

func (r *InvokeRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("invokerun", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InvokeRun")
...
func (r *InvokeRunReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataflowv1alpha1.InvokeRun{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataflowv1alpha1.InvokeRun{}.APIVersion, dataflowv1alpha1.InvokeRun{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataflowv1alpha1.InvokeRun{}).
