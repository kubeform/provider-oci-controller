...
// +kubebuilder:rbac:groups=dataflow.oci.kubeform.com,resources=privateendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataflow.oci.kubeform.com,resources=privateendpoints/status,verbs=get;update;patch

func (r *PrivateEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privateendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateEndpoint")
...
func (r *PrivateEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataflowv1alpha1.PrivateEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataflowv1alpha1.PrivateEndpoint{}.APIVersion, dataflowv1alpha1.PrivateEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataflowv1alpha1.PrivateEndpoint{}).
