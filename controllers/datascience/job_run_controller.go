...
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=jobruns,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=jobruns/status,verbs=get;update;patch
...
func (r *JobRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobRun")
...
func (r *JobRunReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasciencev1alpha1.JobRun{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasciencev1alpha1.JobRun{}.APIVersion, datasciencev1alpha1.JobRun{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasciencev1alpha1.JobRun{}).
