...
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=modeldeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=modeldeployments/status,verbs=get;update;patch

func (r *ModelDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("modeldeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ModelDeployment")
...
func (r *ModelDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasciencev1alpha1.ModelDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasciencev1alpha1.ModelDeployment{}.APIVersion, datasciencev1alpha1.ModelDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasciencev1alpha1.ModelDeployment{}).
