...
// ModelProvenanceReconciler reconciles a ModelProvenance object
type ModelProvenanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=modelprovenances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=modelprovenances/status,verbs=get;update;patch

func (r *ModelProvenanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("modelprovenance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ModelProvenance")
...
func (r *ModelProvenanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasciencev1alpha1.ModelProvenance{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasciencev1alpha1.ModelProvenance{}.APIVersion, datasciencev1alpha1.ModelProvenance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasciencev1alpha1.ModelProvenance{}).
