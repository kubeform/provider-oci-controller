...
// NotebookSessionReconciler reconciles a NotebookSession object
type NotebookSessionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=notebooksessions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datascience.oci.kubeform.com,resources=notebooksessions/status,verbs=get;update;patch

func (r *NotebookSessionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("notebooksession", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NotebookSession")
...
func (r *NotebookSessionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasciencev1alpha1.NotebookSession{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasciencev1alpha1.NotebookSession{}.APIVersion, datasciencev1alpha1.NotebookSession{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasciencev1alpha1.NotebookSession{}).
