...
// BuildPipelineStageReconciler reconciles a BuildPipelineStage object
type BuildPipelineStageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=buildpipelinestages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=buildpipelinestages/status,verbs=get;update;patch

func (r *BuildPipelineStageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("buildpipelinestage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BuildPipelineStage")
...
func (r *BuildPipelineStageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.BuildPipelineStage{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.BuildPipelineStage{}.APIVersion, devopsv1alpha1.BuildPipelineStage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.BuildPipelineStage{}).
