...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deployartifacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deployartifacts/status,verbs=get;update;patch

func (r *DeployArtifactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deployartifact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeployArtifact")
...
func (r *DeployArtifactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.DeployArtifact{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.DeployArtifact{}.APIVersion, devopsv1alpha1.DeployArtifact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.DeployArtifact{}).
