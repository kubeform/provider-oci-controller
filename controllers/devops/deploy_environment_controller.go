...
// DeployEnvironmentReconciler reconciles a DeployEnvironment object
type DeployEnvironmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deployenvironments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deployenvironments/status,verbs=get;update;patch

func (r *DeployEnvironmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deployenvironment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeployEnvironment")
...
func (r *DeployEnvironmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.DeployEnvironment{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.DeployEnvironment{}.APIVersion, devopsv1alpha1.DeployEnvironment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.DeployEnvironment{}).
