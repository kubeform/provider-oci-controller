...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deploypipelines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deploypipelines/status,verbs=get;update;patch

func (r *DeployPipelineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deploypipeline", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeployPipeline")
...
func (r *DeployPipelineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.DeployPipeline{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.DeployPipeline{}.APIVersion, devopsv1alpha1.DeployPipeline{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.DeployPipeline{}).
