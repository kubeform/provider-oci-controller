...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deploystages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=deploystages/status,verbs=get;update;patch

func (r *DeployStageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deploystage", req.NamespacedName)
...
func (r *DeployStageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.DeployStage{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.DeployStage{}.APIVersion, devopsv1alpha1.DeployStage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.DeployStage{}).
