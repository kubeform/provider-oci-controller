...
// RepositoryRefReconciler reconciles a RepositoryRef object
...
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=repositoryrefs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devops.oci.kubeform.com,resources=repositoryrefs/status,verbs=get;update;patch

func (r *RepositoryRefReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("repositoryref", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RepositoryRef")
...
func (r *RepositoryRefReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devopsv1alpha1.RepositoryRef{}); err != nil {
			klog.Error(err, "unable to set up auditor", devopsv1alpha1.RepositoryRef{}.APIVersion, devopsv1alpha1.RepositoryRef{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devopsv1alpha1.RepositoryRef{}).
