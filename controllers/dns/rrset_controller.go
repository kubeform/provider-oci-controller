...
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=rrsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=rrsets/status,verbs=get;update;patch

func (r *RrsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rrset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Rrset")
...
func (r *RrsetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.Rrset{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.Rrset{}.APIVersion, dnsv1alpha1.Rrset{}.Kind)
			return err
		}
	}
