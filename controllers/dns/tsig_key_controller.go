...
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=tsigkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=tsigkeys/status,verbs=get;update;patch

func (r *TsigKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tsigkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TsigKey")
...
func (r *TsigKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.TsigKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.TsigKey{}.APIVersion, dnsv1alpha1.TsigKey{}.Kind)
			return err
		}
	}
...
func (r *TsigKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for TsigKey")
			return nil
		}
		...
		resourceList := &dnsv1alpha1.TsigKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list TsigKey")
			return nil
		}
