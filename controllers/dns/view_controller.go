...
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=views,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.oci.kubeform.com,resources=views/status,verbs=get;update;patch

func (r *ViewReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("view", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch View")
...
func (r *ViewReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.View{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.View{}.APIVersion, dnsv1alpha1.View{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.View{}).
