...
// +kubebuilder:rbac:groups=email.oci.kubeform.com,resources=emaildomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=email.oci.kubeform.com,resources=emaildomains/status,verbs=get;update;patch

func (r *EmailDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("emaildomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EmailDomain")
...
func (r *EmailDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &emailv1alpha1.EmailDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", emailv1alpha1.EmailDomain{}.APIVersion, emailv1alpha1.EmailDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&emailv1alpha1.EmailDomain{}).
