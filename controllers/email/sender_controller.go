...
// +kubebuilder:rbac:groups=email.oci.kubeform.com,resources=senders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=email.oci.kubeform.com,resources=senders/status,verbs=get;update;patch

func (r *SenderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sender", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Sender")
...
func (r *SenderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &emailv1alpha1.Sender{}); err != nil {
			klog.Error(err, "unable to set up auditor", emailv1alpha1.Sender{}.APIVersion, emailv1alpha1.Sender{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&emailv1alpha1.Sender{}).
