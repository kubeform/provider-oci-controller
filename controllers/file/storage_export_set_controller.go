...
// StorageExportSetReconciler reconciles a StorageExportSet object
type StorageExportSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storageexportsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storageexportsets/status,verbs=get;update;patch

func (r *StorageExportSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storageexportset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageExportSet")
...
func (r *StorageExportSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &filev1alpha1.StorageExportSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", filev1alpha1.StorageExportSet{}.APIVersion, filev1alpha1.StorageExportSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&filev1alpha1.StorageExportSet{}).
