...
// StorageMountTargetReconciler reconciles a StorageMountTarget object
type StorageMountTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storagemounttargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storagemounttargets/status,verbs=get;update;patch

func (r *StorageMountTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagemounttarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageMountTarget")
...
func (r *StorageMountTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &filev1alpha1.StorageMountTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", filev1alpha1.StorageMountTarget{}.APIVersion, filev1alpha1.StorageMountTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&filev1alpha1.StorageMountTarget{}).
