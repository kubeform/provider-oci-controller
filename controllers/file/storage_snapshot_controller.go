...
// StorageSnapshotReconciler reconciles a StorageSnapshot object
type StorageSnapshotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storagesnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=file.oci.kubeform.com,resources=storagesnapshots/status,verbs=get;update;patch

func (r *StorageSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagesnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageSnapshot")
...
func (r *StorageSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &filev1alpha1.StorageSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", filev1alpha1.StorageSnapshot{}.APIVersion, filev1alpha1.StorageSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&filev1alpha1.StorageSnapshot{}).
