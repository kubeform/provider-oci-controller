...
// InvokeFunctionReconciler reconciles a InvokeFunction object
type InvokeFunctionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=functions.oci.kubeform.com,resources=invokefunctions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=functions.oci.kubeform.com,resources=invokefunctions/status,verbs=get;update;patch

func (r *InvokeFunctionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("invokefunction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InvokeFunction")
...
func (r *InvokeFunctionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &functionsv1alpha1.InvokeFunction{}); err != nil {
			klog.Error(err, "unable to set up auditor", functionsv1alpha1.InvokeFunction{}.APIVersion, functionsv1alpha1.InvokeFunction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&functionsv1alpha1.InvokeFunction{}).
