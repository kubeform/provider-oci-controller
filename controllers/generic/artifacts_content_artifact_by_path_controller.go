...
// ArtifactsContentArtifactByPathReconciler reconciles a ArtifactsContentArtifactByPath object
type ArtifactsContentArtifactByPathReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=generic.oci.kubeform.com,resources=artifactscontentartifactbypaths,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=generic.oci.kubeform.com,resources=artifactscontentartifactbypaths/status,verbs=get;update;patch

func (r *ArtifactsContentArtifactByPathReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("artifactscontentartifactbypath", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ArtifactsContentArtifactByPath")
...
func (r *ArtifactsContentArtifactByPathReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &genericv1alpha1.ArtifactsContentArtifactByPath{}); err != nil {
			klog.Error(err, "unable to set up auditor", genericv1alpha1.ArtifactsContentArtifactByPath{}.APIVersion, genericv1alpha1.ArtifactsContentArtifactByPath{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&genericv1alpha1.ArtifactsContentArtifactByPath{}).
