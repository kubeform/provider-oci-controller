...
// GateDeploymentBackupReconciler reconciles a GateDeploymentBackup object
type GateDeploymentBackupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=golden.oci.kubeform.com,resources=gatedeploymentbackups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=golden.oci.kubeform.com,resources=gatedeploymentbackups/status,verbs=get;update;patch

func (r *GateDeploymentBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatedeploymentbackup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GateDeploymentBackup")
...
func (r *GateDeploymentBackupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &goldenv1alpha1.GateDeploymentBackup{}); err != nil {
			klog.Error(err, "unable to set up auditor", goldenv1alpha1.GateDeploymentBackup{}.APIVersion, goldenv1alpha1.GateDeploymentBackup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&goldenv1alpha1.GateDeploymentBackup{}).
