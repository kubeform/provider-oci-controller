...
// GateDeploymentReconciler reconciles a GateDeployment object
type GateDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=golden.oci.kubeform.com,resources=gatedeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=golden.oci.kubeform.com,resources=gatedeployments/status,verbs=get;update;patch

func (r *GateDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatedeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GateDeployment")
...
func (r *GateDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &goldenv1alpha1.GateDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", goldenv1alpha1.GateDeployment{}.APIVersion, goldenv1alpha1.GateDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&goldenv1alpha1.GateDeployment{}).
...
func (r *GateDeploymentReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for GateDeployment")
			return nil
		}
		...
		resourceList := &goldenv1alpha1.GateDeploymentList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list GateDeployment")
			return nil
		}
