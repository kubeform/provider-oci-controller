...
// ChecksHTTPMonitorReconciler reconciles a ChecksHTTPMonitor object
type ChecksHTTPMonitorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=health.oci.kubeform.com,resources=checkshttpmonitors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=health.oci.kubeform.com,resources=checkshttpmonitors/status,verbs=get;update;patch

func (r *ChecksHTTPMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("checkshttpmonitor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChecksHTTPMonitor")
...
func (r *ChecksHTTPMonitorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthv1alpha1.ChecksHTTPMonitor{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthv1alpha1.ChecksHTTPMonitor{}.APIVersion, healthv1alpha1.ChecksHTTPMonitor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthv1alpha1.ChecksHTTPMonitor{}).
