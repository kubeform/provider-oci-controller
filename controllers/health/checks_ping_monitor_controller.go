...
// ChecksPingMonitorReconciler reconciles a ChecksPingMonitor object
type ChecksPingMonitorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=health.oci.kubeform.com,resources=checkspingmonitors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=health.oci.kubeform.com,resources=checkspingmonitors/status,verbs=get;update;patch

func (r *ChecksPingMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("checkspingmonitor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChecksPingMonitor")
...
func (r *ChecksPingMonitorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthv1alpha1.ChecksPingMonitor{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthv1alpha1.ChecksPingMonitor{}.APIVersion, healthv1alpha1.ChecksPingMonitor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthv1alpha1.ChecksPingMonitor{}).
