...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=authtokens,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=authtokens/status,verbs=get;update;patch

func (r *AuthTokenReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("authtoken", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AuthToken")
...
func (r *AuthTokenReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.AuthToken{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.AuthToken{}.APIVersion, identityv1alpha1.AuthToken{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.AuthToken{}).
