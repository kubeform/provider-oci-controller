...
// DomainReplicationToRegionReconciler reconciles a DomainReplicationToRegion object
type DomainReplicationToRegionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=domainreplicationtoregions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=domainreplicationtoregions/status,verbs=get;update;patch

func (r *DomainReplicationToRegionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainreplicationtoregion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainReplicationToRegion")
...
func (r *DomainReplicationToRegionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.DomainReplicationToRegion{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.DomainReplicationToRegion{}.APIVersion, identityv1alpha1.DomainReplicationToRegion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.DomainReplicationToRegion{}).
