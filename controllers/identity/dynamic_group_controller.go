...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=dynamicgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=dynamicgroups/status,verbs=get;update;patch

func (r *DynamicGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dynamicgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DynamicGroup")
...
func (r *DynamicGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.DynamicGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.DynamicGroup{}.APIVersion, identityv1alpha1.DynamicGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.DynamicGroup{}).
