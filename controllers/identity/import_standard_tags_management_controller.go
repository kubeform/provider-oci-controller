...
// ImportStandardTagsManagementReconciler reconciles a ImportStandardTagsManagement object
type ImportStandardTagsManagementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=importstandardtagsmanagements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=importstandardtagsmanagements/status,verbs=get;update;patch

func (r *ImportStandardTagsManagementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("importstandardtagsmanagement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ImportStandardTagsManagement")
...
func (r *ImportStandardTagsManagementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.ImportStandardTagsManagement{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.ImportStandardTagsManagement{}.APIVersion, identityv1alpha1.ImportStandardTagsManagement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.ImportStandardTagsManagement{}).
