...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=networksources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=networksources/status,verbs=get;update;patch

func (r *NetworkSourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networksource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkSource")
...
func (r *NetworkSourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.NetworkSource{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.NetworkSource{}.APIVersion, identityv1alpha1.NetworkSource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.NetworkSource{}).
