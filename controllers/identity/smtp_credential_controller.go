...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=smtpcredentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=smtpcredentials/status,verbs=get;update;patch

func (r *SmtpCredentialReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("smtpcredential", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SmtpCredential")
...
func (r *SmtpCredentialReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.SmtpCredential{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.SmtpCredential{}.APIVersion, identityv1alpha1.SmtpCredential{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.SmtpCredential{}).
