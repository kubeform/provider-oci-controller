...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=swiftpasswords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=swiftpasswords/status,verbs=get;update;patch

func (r *SwiftPasswordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("swiftpassword", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SwiftPassword")
...
func (r *SwiftPasswordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.SwiftPassword{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.SwiftPassword{}.APIVersion, identityv1alpha1.SwiftPassword{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.SwiftPassword{}).
