...
// UiPasswordReconciler reconciles a UiPassword object
type UiPasswordReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=uipasswords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=uipasswords/status,verbs=get;update;patch

func (r *UiPasswordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("uipassword", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UiPassword")
...
func (r *UiPasswordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.UiPassword{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.UiPassword{}.APIVersion, identityv1alpha1.UiPassword{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.UiPassword{}).
