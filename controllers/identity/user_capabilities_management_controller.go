...
// UserCapabilitiesManagementReconciler reconciles a UserCapabilitiesManagement object
type UserCapabilitiesManagementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=usercapabilitiesmanagements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.oci.kubeform.com,resources=usercapabilitiesmanagements/status,verbs=get;update;patch

func (r *UserCapabilitiesManagementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("usercapabilitiesmanagement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserCapabilitiesManagement")
...
func (r *UserCapabilitiesManagementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.UserCapabilitiesManagement{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.UserCapabilitiesManagement{}.APIVersion, identityv1alpha1.UserCapabilitiesManagement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.UserCapabilitiesManagement{}).
