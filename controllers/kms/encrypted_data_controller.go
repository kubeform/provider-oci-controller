...
// EncryptedDataReconciler reconciles a EncryptedData object
type EncryptedDataReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=encrypteddata,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=encrypteddata/status,verbs=get;update;patch

func (r *EncryptedDataReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("encrypteddata", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EncryptedData")
...
func (r *EncryptedDataReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.EncryptedData{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.EncryptedData{}.APIVersion, kmsv1alpha1.EncryptedData{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.EncryptedData{}).
