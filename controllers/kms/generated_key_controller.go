...
// GeneratedKeyReconciler reconciles a GeneratedKey object
type GeneratedKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=generatedkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=generatedkeys/status,verbs=get;update;patch

func (r *GeneratedKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("generatedkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GeneratedKey")
...
func (r *GeneratedKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.GeneratedKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.GeneratedKey{}.APIVersion, kmsv1alpha1.GeneratedKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.GeneratedKey{}).
