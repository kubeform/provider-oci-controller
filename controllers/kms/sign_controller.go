...
// SignReconciler reconciles a Sign object
type SignReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=signs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=signs/status,verbs=get;update;patch

func (r *SignReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sign", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Sign")
...
func (r *SignReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.Sign{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.Sign{}.APIVersion, kmsv1alpha1.Sign{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.Sign{}).
