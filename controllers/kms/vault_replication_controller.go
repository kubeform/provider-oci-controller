...
// VaultReplicationReconciler reconciles a VaultReplication object
type VaultReplicationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=vaultreplications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.oci.kubeform.com,resources=vaultreplications/status,verbs=get;update;patch

func (r *VaultReplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vaultreplication", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VaultReplication")
...
func (r *VaultReplicationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.VaultReplication{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.VaultReplication{}.APIVersion, kmsv1alpha1.VaultReplication{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.VaultReplication{}).
