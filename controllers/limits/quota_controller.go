...
// +kubebuilder:rbac:groups=limits.oci.kubeform.com,resources=quotas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=limits.oci.kubeform.com,resources=quotas/status,verbs=get;update;patch

func (r *QuotaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("quota", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Quota")
...
func (r *QuotaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &limitsv1alpha1.Quota{}); err != nil {
			klog.Error(err, "unable to set up auditor", limitsv1alpha1.Quota{}.APIVersion, limitsv1alpha1.Quota{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&limitsv1alpha1.Quota{}).
