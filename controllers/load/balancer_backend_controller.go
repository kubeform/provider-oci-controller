...
// BalancerBackendReconciler reconciles a BalancerBackend object
type BalancerBackendReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerbackends,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerbackends/status,verbs=get;update;patch

func (r *BalancerBackendReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerbackend", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerBackend")
...
func (r *BalancerBackendReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerBackend{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerBackend{}.APIVersion, loadv1alpha1.BalancerBackend{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerBackend{}).
