...
// BalancerBackendsetReconciler reconciles a BalancerBackendset object
type BalancerBackendsetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerbackendsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerbackendsets/status,verbs=get;update;patch

func (r *BalancerBackendsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerbackendset", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerBackendset")
...
func (r *BalancerBackendsetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerBackendset{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerBackendset{}.APIVersion, loadv1alpha1.BalancerBackendset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerBackendset{}).
