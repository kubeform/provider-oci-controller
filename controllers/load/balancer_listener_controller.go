...
// BalancerListenerReconciler reconciles a BalancerListener object
type BalancerListenerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerlisteners,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerlisteners/status,verbs=get;update;patch

func (r *BalancerListenerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerlistener", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerListener")
...
func (r *BalancerListenerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerListener{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerListener{}.APIVersion, loadv1alpha1.BalancerListener{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerListener{}).
