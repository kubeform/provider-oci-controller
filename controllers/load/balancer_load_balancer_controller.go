...
// BalancerLoadBalancerReconciler reconciles a BalancerLoadBalancer object
type BalancerLoadBalancerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerloadbalancers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerloadbalancers/status,verbs=get;update;patch

func (r *BalancerLoadBalancerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerloadbalancer", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerLoadBalancer")
...
func (r *BalancerLoadBalancerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerLoadBalancer{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerLoadBalancer{}.APIVersion, loadv1alpha1.BalancerLoadBalancer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerLoadBalancer{}).
