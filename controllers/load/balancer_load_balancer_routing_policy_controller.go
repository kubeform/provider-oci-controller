...
// BalancerLoadBalancerRoutingPolicyReconciler reconciles a BalancerLoadBalancerRoutingPolicy object
type BalancerLoadBalancerRoutingPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerloadbalancerroutingpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerloadbalancerroutingpolicies/status,verbs=get;update;patch

func (r *BalancerLoadBalancerRoutingPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerloadbalancerroutingpolicy", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerLoadBalancerRoutingPolicy")
...
func (r *BalancerLoadBalancerRoutingPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerLoadBalancerRoutingPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerLoadBalancerRoutingPolicy{}.APIVersion, loadv1alpha1.BalancerLoadBalancerRoutingPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerLoadBalancerRoutingPolicy{}).
