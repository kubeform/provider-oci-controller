...
// BalancerPathRouteSetReconciler reconciles a BalancerPathRouteSet object
type BalancerPathRouteSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerpathroutesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerpathroutesets/status,verbs=get;update;patch

func (r *BalancerPathRouteSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerpathrouteset", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerPathRouteSet")
...
func (r *BalancerPathRouteSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerPathRouteSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerPathRouteSet{}.APIVersion, loadv1alpha1.BalancerPathRouteSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerPathRouteSet{}).
