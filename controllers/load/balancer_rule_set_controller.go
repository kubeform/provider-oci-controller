...
// BalancerRuleSetReconciler reconciles a BalancerRuleSet object
type BalancerRuleSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerrulesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancerrulesets/status,verbs=get;update;patch

func (r *BalancerRuleSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerruleset", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerRuleSet")
...
func (r *BalancerRuleSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerRuleSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerRuleSet{}.APIVersion, loadv1alpha1.BalancerRuleSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerRuleSet{}).
