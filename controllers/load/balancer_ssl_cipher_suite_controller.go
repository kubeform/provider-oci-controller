...
// BalancerSslCipherSuiteReconciler reconciles a BalancerSslCipherSuite object
type BalancerSslCipherSuiteReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancersslciphersuites,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.oci.kubeform.com,resources=balancersslciphersuites/status,verbs=get;update;patch

func (r *BalancerSslCipherSuiteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancersslciphersuite", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerSslCipherSuite")
...
func (r *BalancerSslCipherSuiteReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerSslCipherSuite{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerSslCipherSuite{}.APIVersion, loadv1alpha1.BalancerSslCipherSuite{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerSslCipherSuite{}).
