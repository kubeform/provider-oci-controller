...
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=backendsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=backendsets/status,verbs=get;update;patch

func (r *BackendSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendSet")
...
func (r *BackendSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadbalancerv1alpha1.BackendSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadbalancerv1alpha1.BackendSet{}.APIVersion, loadbalancerv1alpha1.BackendSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadbalancerv1alpha1.BackendSet{}).
