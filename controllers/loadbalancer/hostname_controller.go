...
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=hostnames,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=hostnames/status,verbs=get;update;patch

func (r *HostnameReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostname", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Hostname")
...
func (r *HostnameReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadbalancerv1alpha1.Hostname{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadbalancerv1alpha1.Hostname{}.APIVersion, loadbalancerv1alpha1.Hostname{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadbalancerv1alpha1.Hostname{}).
