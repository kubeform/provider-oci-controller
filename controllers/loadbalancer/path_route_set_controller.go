...
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=pathroutesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=pathroutesets/status,verbs=get;update;patch

func (r *PathRouteSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pathrouteset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PathRouteSet")
...
func (r *PathRouteSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadbalancerv1alpha1.PathRouteSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadbalancerv1alpha1.PathRouteSet{}.APIVersion, loadbalancerv1alpha1.PathRouteSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadbalancerv1alpha1.PathRouteSet{}).
