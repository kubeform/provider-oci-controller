...
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=sslciphersuites,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loadbalancer.oci.kubeform.com,resources=sslciphersuites/status,verbs=get;update;patch

func (r *SslCipherSuiteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sslciphersuite", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SslCipherSuite")
...
func (r *SslCipherSuiteReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadbalancerv1alpha1.SslCipherSuite{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadbalancerv1alpha1.SslCipherSuite{}.APIVersion, loadbalancerv1alpha1.SslCipherSuite{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadbalancerv1alpha1.SslCipherSuite{}).
