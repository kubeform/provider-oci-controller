...
// AnalyticsLogAnalyticsEntityReconciler reconciles a AnalyticsLogAnalyticsEntity object
type AnalyticsLogAnalyticsEntityReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsloganalyticsentities,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsloganalyticsentities/status,verbs=get;update;patch

func (r *AnalyticsLogAnalyticsEntityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsloganalyticsentity", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsLogAnalyticsEntity")
...
func (r *AnalyticsLogAnalyticsEntityReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.AnalyticsLogAnalyticsEntity{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.AnalyticsLogAnalyticsEntity{}.APIVersion, logv1alpha1.AnalyticsLogAnalyticsEntity{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.AnalyticsLogAnalyticsEntity{}).
