...
// AnalyticsNamespaceReconciler reconciles a AnalyticsNamespace object
type AnalyticsNamespaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsnamespaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsnamespaces/status,verbs=get;update;patch

func (r *AnalyticsNamespaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsnamespace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsNamespace")
...
func (r *AnalyticsNamespaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.AnalyticsNamespace{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.AnalyticsNamespace{}.APIVersion, logv1alpha1.AnalyticsNamespace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.AnalyticsNamespace{}).
