...
// AnalyticsNamespaceScheduledTaskReconciler reconciles a AnalyticsNamespaceScheduledTask object
type AnalyticsNamespaceScheduledTaskReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsnamespacescheduledtasks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.oci.kubeform.com,resources=analyticsnamespacescheduledtasks/status,verbs=get;update;patch

func (r *AnalyticsNamespaceScheduledTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyticsnamespacescheduledtask", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnalyticsNamespaceScheduledTask")
...
func (r *AnalyticsNamespaceScheduledTaskReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.AnalyticsNamespaceScheduledTask{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.AnalyticsNamespaceScheduledTask{}.APIVersion, logv1alpha1.AnalyticsNamespaceScheduledTask{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.AnalyticsNamespaceScheduledTask{}).
