...
// +kubebuilder:rbac:groups=logging.oci.kubeform.com,resources=logsavedsearches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.oci.kubeform.com,resources=logsavedsearches/status,verbs=get;update;patch

func (r *LogSavedSearchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("logsavedsearch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LogSavedSearch")
...
func (r *LogSavedSearchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.LogSavedSearch{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.LogSavedSearch{}.APIVersion, loggingv1alpha1.LogSavedSearch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.LogSavedSearch{}).
