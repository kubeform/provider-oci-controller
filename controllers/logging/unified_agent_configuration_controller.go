...
// +kubebuilder:rbac:groups=logging.oci.kubeform.com,resources=unifiedagentconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.oci.kubeform.com,resources=unifiedagentconfigurations/status,verbs=get;update;patch

func (r *UnifiedAgentConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("unifiedagentconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UnifiedAgentConfiguration")
...
func (r *UnifiedAgentConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.UnifiedAgentConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.UnifiedAgentConfiguration{}.APIVersion, loggingv1alpha1.UnifiedAgentConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.UnifiedAgentConfiguration{}).
