...
// AgentManagementAgentReconciler reconciles a AgentManagementAgent object
type AgentManagementAgentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=management.oci.kubeform.com,resources=agentmanagementagents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=management.oci.kubeform.com,resources=agentmanagementagents/status,verbs=get;update;patch

func (r *AgentManagementAgentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("agentmanagementagent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AgentManagementAgent")
...
func (r *AgentManagementAgentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managementv1alpha1.AgentManagementAgent{}); err != nil {
			klog.Error(err, "unable to set up auditor", managementv1alpha1.AgentManagementAgent{}.APIVersion, managementv1alpha1.AgentManagementAgent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managementv1alpha1.AgentManagementAgent{}).
