...
// AgentManagementAgentInstallKeyReconciler reconciles a AgentManagementAgentInstallKey object
type AgentManagementAgentInstallKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=management.oci.kubeform.com,resources=agentmanagementagentinstallkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=management.oci.kubeform.com,resources=agentmanagementagentinstallkeys/status,verbs=get;update;patch

func (r *AgentManagementAgentInstallKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("agentmanagementagentinstallkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AgentManagementAgentInstallKey")
...
func (r *AgentManagementAgentInstallKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &managementv1alpha1.AgentManagementAgentInstallKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", managementv1alpha1.AgentManagementAgentInstallKey{}.APIVersion, managementv1alpha1.AgentManagementAgentInstallKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&managementv1alpha1.AgentManagementAgentInstallKey{}).
