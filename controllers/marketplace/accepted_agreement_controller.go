...
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=acceptedagreements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=acceptedagreements/status,verbs=get;update;patch

func (r *AcceptedAgreementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("acceptedagreement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AcceptedAgreement")
...
func (r *AcceptedAgreementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &marketplacev1alpha1.AcceptedAgreement{}); err != nil {
			klog.Error(err, "unable to set up auditor", marketplacev1alpha1.AcceptedAgreement{}.APIVersion, marketplacev1alpha1.AcceptedAgreement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&marketplacev1alpha1.AcceptedAgreement{}).
