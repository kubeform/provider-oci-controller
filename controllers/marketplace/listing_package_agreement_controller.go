...
// ListingPackageAgreementReconciler reconciles a ListingPackageAgreement object
type ListingPackageAgreementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=listingpackageagreements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=listingpackageagreements/status,verbs=get;update;patch

func (r *ListingPackageAgreementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("listingpackageagreement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ListingPackageAgreement")
...
func (r *ListingPackageAgreementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &marketplacev1alpha1.ListingPackageAgreement{}); err != nil {
			klog.Error(err, "unable to set up auditor", marketplacev1alpha1.ListingPackageAgreement{}.APIVersion, marketplacev1alpha1.ListingPackageAgreement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&marketplacev1alpha1.ListingPackageAgreement{}).
