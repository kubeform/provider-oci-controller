...
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=publications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=marketplace.oci.kubeform.com,resources=publications/status,verbs=get;update;patch

func (r *PublicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("publication", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Publication")
...
func (r *PublicationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &marketplacev1alpha1.Publication{}); err != nil {
			klog.Error(err, "unable to set up auditor", marketplacev1alpha1.Publication{}.APIVersion, marketplacev1alpha1.Publication{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&marketplacev1alpha1.Publication{}).
