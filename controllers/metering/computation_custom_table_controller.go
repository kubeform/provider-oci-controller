...
// ComputationCustomTableReconciler reconciles a ComputationCustomTable object
type ComputationCustomTableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metering.oci.kubeform.com,resources=computationcustomtables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metering.oci.kubeform.com,resources=computationcustomtables/status,verbs=get;update;patch

func (r *ComputationCustomTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("computationcustomtable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ComputationCustomTable")
...
func (r *ComputationCustomTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &meteringv1alpha1.ComputationCustomTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", meteringv1alpha1.ComputationCustomTable{}.APIVersion, meteringv1alpha1.ComputationCustomTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&meteringv1alpha1.ComputationCustomTable{}).
