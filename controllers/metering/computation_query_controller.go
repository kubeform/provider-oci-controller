...
// ComputationQueryReconciler reconciles a ComputationQuery object
type ComputationQueryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metering.oci.kubeform.com,resources=computationqueries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metering.oci.kubeform.com,resources=computationqueries/status,verbs=get;update;patch

func (r *ComputationQueryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("computationquery", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ComputationQuery")
...
func (r *ComputationQueryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &meteringv1alpha1.ComputationQuery{}); err != nil {
			klog.Error(err, "unable to set up auditor", meteringv1alpha1.ComputationQuery{}.APIVersion, meteringv1alpha1.ComputationQuery{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&meteringv1alpha1.ComputationQuery{}).
