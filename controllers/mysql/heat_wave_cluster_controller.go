...
// +kubebuilder:rbac:groups=mysql.oci.kubeform.com,resources=heatwaveclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mysql.oci.kubeform.com,resources=heatwaveclusters/status,verbs=get;update;patch

func (r *HeatWaveClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("heatwavecluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HeatWaveCluster")
...
func (r *HeatWaveClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mysqlv1alpha1.HeatWaveCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", mysqlv1alpha1.HeatWaveCluster{}.APIVersion, mysqlv1alpha1.HeatWaveCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mysqlv1alpha1.HeatWaveCluster{}).
