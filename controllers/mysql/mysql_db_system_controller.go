...
// +kubebuilder:rbac:groups=mysql.oci.kubeform.com,resources=mysqldbsystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mysql.oci.kubeform.com,resources=mysqldbsystems/status,verbs=get;update;patch

func (r *MysqlDbSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mysqldbsystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MysqlDbSystem")
...
func (r *MysqlDbSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mysqlv1alpha1.MysqlDbSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", mysqlv1alpha1.MysqlDbSystem{}.APIVersion, mysqlv1alpha1.MysqlDbSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mysqlv1alpha1.MysqlDbSystem{}).
...
func (r *MysqlDbSystemReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for MysqlDbSystem")
			return nil
		}
		...
		resourceList := &mysqlv1alpha1.MysqlDbSystemList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MysqlDbSystem")
			return nil
		}
