...
// LoadBalancerBackendReconciler reconciles a LoadBalancerBackend object
type LoadBalancerBackendReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.oci.kubeform.com,resources=loadbalancerbackends,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.oci.kubeform.com,resources=loadbalancerbackends/status,verbs=get;update;patch

func (r *LoadBalancerBackendReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("loadbalancerbackend", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LoadBalancerBackend")
...
func (r *LoadBalancerBackendReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.LoadBalancerBackend{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.LoadBalancerBackend{}.APIVersion, networkv1alpha1.LoadBalancerBackend{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.LoadBalancerBackend{}).
