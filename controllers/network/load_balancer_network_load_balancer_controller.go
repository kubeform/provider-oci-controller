...
// LoadBalancerNetworkLoadBalancerReconciler reconciles a LoadBalancerNetworkLoadBalancer object
type LoadBalancerNetworkLoadBalancerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.oci.kubeform.com,resources=loadbalancernetworkloadbalancers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.oci.kubeform.com,resources=loadbalancernetworkloadbalancers/status,verbs=get;update;patch

func (r *LoadBalancerNetworkLoadBalancerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("loadbalancernetworkloadbalancer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LoadBalancerNetworkLoadBalancer")
...
func (r *LoadBalancerNetworkLoadBalancerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.LoadBalancerNetworkLoadBalancer{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.LoadBalancerNetworkLoadBalancer{}.APIVersion, networkv1alpha1.LoadBalancerNetworkLoadBalancer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.LoadBalancerNetworkLoadBalancer{}).
