...
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=objectlifecyclepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=objectlifecyclepolicies/status,verbs=get;update;patch

func (r *ObjectLifecyclePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("objectlifecyclepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ObjectLifecyclePolicy")
...
func (r *ObjectLifecyclePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &objectstoragev1alpha1.ObjectLifecyclePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", objectstoragev1alpha1.ObjectLifecyclePolicy{}.APIVersion, objectstoragev1alpha1.ObjectLifecyclePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&objectstoragev1alpha1.ObjectLifecyclePolicy{}).
