...
// PreauthrequestReconciler reconciles a Preauthrequest object
type PreauthrequestReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=preauthrequests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=preauthrequests/status,verbs=get;update;patch

func (r *PreauthrequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("preauthrequest", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Preauthrequest")
...
func (r *PreauthrequestReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &objectstoragev1alpha1.Preauthrequest{}); err != nil {
			klog.Error(err, "unable to set up auditor", objectstoragev1alpha1.Preauthrequest{}.APIVersion, objectstoragev1alpha1.Preauthrequest{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&objectstoragev1alpha1.Preauthrequest{}).
