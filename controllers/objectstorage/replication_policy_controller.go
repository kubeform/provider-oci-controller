...
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=replicationpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=objectstorage.oci.kubeform.com,resources=replicationpolicies/status,verbs=get;update;patch

func (r *ReplicationPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicationpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicationPolicy")
...
func (r *ReplicationPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &objectstoragev1alpha1.ReplicationPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", objectstoragev1alpha1.ReplicationPolicy{}.APIVersion, objectstoragev1alpha1.ReplicationPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&objectstoragev1alpha1.ReplicationPolicy{}).
