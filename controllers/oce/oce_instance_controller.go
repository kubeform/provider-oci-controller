...
// +kubebuilder:rbac:groups=oce.oci.kubeform.com,resources=oceinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oce.oci.kubeform.com,resources=oceinstances/status,verbs=get;update;patch

func (r *OceInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("oceinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OceInstance")
...
func (r *OceInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ocev1alpha1.OceInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", ocev1alpha1.OceInstance{}.APIVersion, ocev1alpha1.OceInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ocev1alpha1.OceInstance{}).
...
func (r *OceInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for OceInstance")
			return nil
		}
		...
		resourceList := &ocev1alpha1.OceInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list OceInstance")
			return nil
		}
