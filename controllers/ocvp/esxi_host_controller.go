...
// +kubebuilder:rbac:groups=ocvp.oci.kubeform.com,resources=esxihosts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ocvp.oci.kubeform.com,resources=esxihosts/status,verbs=get;update;patch

func (r *EsxiHostReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("esxihost", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EsxiHost")
...
func (r *EsxiHostReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ocvpv1alpha1.EsxiHost{}); err != nil {
			klog.Error(err, "unable to set up auditor", ocvpv1alpha1.EsxiHost{}.APIVersion, ocvpv1alpha1.EsxiHost{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ocvpv1alpha1.EsxiHost{}).
