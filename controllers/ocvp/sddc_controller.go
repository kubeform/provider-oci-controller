...
// +kubebuilder:rbac:groups=ocvp.oci.kubeform.com,resources=sddcs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ocvp.oci.kubeform.com,resources=sddcs/status,verbs=get;update;patch

func (r *SddcReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sddc", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Sddc")
...
func (r *SddcReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ocvpv1alpha1.Sddc{}); err != nil {
			klog.Error(err, "unable to set up auditor", ocvpv1alpha1.Sddc{}.APIVersion, ocvpv1alpha1.Sddc{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ocvpv1alpha1.Sddc{}).
