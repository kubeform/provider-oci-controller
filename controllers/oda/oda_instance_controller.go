...
// +kubebuilder:rbac:groups=oda.oci.kubeform.com,resources=odainstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oda.oci.kubeform.com,resources=odainstances/status,verbs=get;update;patch

func (r *OdaInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("odainstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OdaInstance")
...
func (r *OdaInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &odav1alpha1.OdaInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", odav1alpha1.OdaInstance{}.APIVersion, odav1alpha1.OdaInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&odav1alpha1.OdaInstance{}).
