...
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=awrhubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=awrhubs/status,verbs=get;update;patch

func (r *AwrHubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("awrhub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AwrHub")
...
func (r *AwrHubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsiv1alpha1.AwrHub{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsiv1alpha1.AwrHub{}.APIVersion, opsiv1alpha1.AwrHub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsiv1alpha1.AwrHub{}).
