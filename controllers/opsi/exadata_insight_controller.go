...
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=exadatainsights,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=exadatainsights/status,verbs=get;update;patch

func (r *ExadataInsightReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("exadatainsight", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExadataInsight")
...
func (r *ExadataInsightReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsiv1alpha1.ExadataInsight{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsiv1alpha1.ExadataInsight{}.APIVersion, opsiv1alpha1.ExadataInsight{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsiv1alpha1.ExadataInsight{}).
