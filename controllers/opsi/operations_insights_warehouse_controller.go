...
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=operationsinsightswarehouses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=operationsinsightswarehouses/status,verbs=get;update;patch

func (r *OperationsInsightsWarehouseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("operationsinsightswarehouse", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OperationsInsightsWarehouse")
...
func (r *OperationsInsightsWarehouseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsiv1alpha1.OperationsInsightsWarehouse{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsiv1alpha1.OperationsInsightsWarehouse{}.APIVersion, opsiv1alpha1.OperationsInsightsWarehouse{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsiv1alpha1.OperationsInsightsWarehouse{}).
