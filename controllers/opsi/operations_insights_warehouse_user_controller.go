...
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=operationsinsightswarehouseusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsi.oci.kubeform.com,resources=operationsinsightswarehouseusers/status,verbs=get;update;patch

func (r *OperationsInsightsWarehouseUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("operationsinsightswarehouseuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OperationsInsightsWarehouseUser")
...
func (r *OperationsInsightsWarehouseUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsiv1alpha1.OperationsInsightsWarehouseUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsiv1alpha1.OperationsInsightsWarehouseUser{}.APIVersion, opsiv1alpha1.OperationsInsightsWarehouseUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsiv1alpha1.OperationsInsightsWarehouseUser{}).
...
func (r *OperationsInsightsWarehouseUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for OperationsInsightsWarehouseUser")
			return nil
		}
		...
		resourceList := &opsiv1alpha1.OperationsInsightsWarehouseUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list OperationsInsightsWarehouseUser")
			return nil
		}
