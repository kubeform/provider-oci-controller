...
// EnrollmentStatusReconciler reconciles a EnrollmentStatus object
type EnrollmentStatusReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=enrollmentstatuses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=enrollmentstatuses/status,verbs=get;update;patch

func (r *EnrollmentStatusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enrollmentstatus", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnrollmentStatus")
...
func (r *EnrollmentStatusReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &optimizerv1alpha1.EnrollmentStatus{}); err != nil {
			klog.Error(err, "unable to set up auditor", optimizerv1alpha1.EnrollmentStatus{}.APIVersion, optimizerv1alpha1.EnrollmentStatus{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&optimizerv1alpha1.EnrollmentStatus{}).
