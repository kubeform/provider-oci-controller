...
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=recommendations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=recommendations/status,verbs=get;update;patch
...
func (r *RecommendationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Recommendation")
...
func (r *RecommendationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &optimizerv1alpha1.Recommendation{}); err != nil {
			klog.Error(err, "unable to set up auditor", optimizerv1alpha1.Recommendation{}.APIVersion, optimizerv1alpha1.Recommendation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&optimizerv1alpha1.Recommendation{}).
