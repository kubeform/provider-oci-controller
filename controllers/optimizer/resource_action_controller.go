...
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=resourceactions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=optimizer.oci.kubeform.com,resources=resourceactions/status,verbs=get;update;patch

func (r *ResourceActionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourceaction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceAction")
...
func (r *ResourceActionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &optimizerv1alpha1.ResourceAction{}); err != nil {
			klog.Error(err, "unable to set up auditor", optimizerv1alpha1.ResourceAction{}.APIVersion, optimizerv1alpha1.ResourceAction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&optimizerv1alpha1.ResourceAction{}).
