...
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=managedinstancegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=managedinstancegroups/status,verbs=get;update;patch

func (r *ManagedInstanceGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedinstancegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedInstanceGroup")
...
func (r *ManagedInstanceGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osmanagementv1alpha1.ManagedInstanceGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", osmanagementv1alpha1.ManagedInstanceGroup{}.APIVersion, osmanagementv1alpha1.ManagedInstanceGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osmanagementv1alpha1.ManagedInstanceGroup{}).
