...
// ManagedInstanceManagementReconciler reconciles a ManagedInstanceManagement object
type ManagedInstanceManagementReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=managedinstancemanagements,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=managedinstancemanagements/status,verbs=get;update;patch

func (r *ManagedInstanceManagementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedinstancemanagement", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedInstanceManagement")
...
func (r *ManagedInstanceManagementReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osmanagementv1alpha1.ManagedInstanceManagement{}); err != nil {
			klog.Error(err, "unable to set up auditor", osmanagementv1alpha1.ManagedInstanceManagement{}.APIVersion, osmanagementv1alpha1.ManagedInstanceManagement{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osmanagementv1alpha1.ManagedInstanceManagement{}).
