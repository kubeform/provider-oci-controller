...
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=softwaresources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=osmanagement.oci.kubeform.com,resources=softwaresources/status,verbs=get;update;patch

func (r *SoftwareSourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("softwaresource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SoftwareSource")
...
func (r *SoftwareSourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osmanagementv1alpha1.SoftwareSource{}); err != nil {
			klog.Error(err, "unable to set up auditor", osmanagementv1alpha1.SoftwareSource{}.APIVersion, osmanagementv1alpha1.SoftwareSource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osmanagementv1alpha1.SoftwareSource{}).
