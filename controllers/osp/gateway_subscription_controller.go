...
// GatewaySubscriptionReconciler reconciles a GatewaySubscription object
type GatewaySubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=osp.oci.kubeform.com,resources=gatewaysubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=osp.oci.kubeform.com,resources=gatewaysubscriptions/status,verbs=get;update;patch

func (r *GatewaySubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewaysubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewaySubscription")
...
func (r *GatewaySubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ospv1alpha1.GatewaySubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", ospv1alpha1.GatewaySubscription{}.APIVersion, ospv1alpha1.GatewaySubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ospv1alpha1.GatewaySubscription{}).
