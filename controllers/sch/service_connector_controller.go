...
// +kubebuilder:rbac:groups=sch.oci.kubeform.com,resources=serviceconnectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sch.oci.kubeform.com,resources=serviceconnectors/status,verbs=get;update;patch

func (r *ServiceConnectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceconnector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceConnector")
...
func (r *ServiceConnectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &schv1alpha1.ServiceConnector{}); err != nil {
			klog.Error(err, "unable to set up auditor", schv1alpha1.ServiceConnector{}.APIVersion, schv1alpha1.ServiceConnector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&schv1alpha1.ServiceConnector{}).
