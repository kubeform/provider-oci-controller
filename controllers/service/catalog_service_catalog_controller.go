...
// CatalogServiceCatalogReconciler reconciles a CatalogServiceCatalog object
type CatalogServiceCatalogReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.oci.kubeform.com,resources=catalogservicecatalogs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.oci.kubeform.com,resources=catalogservicecatalogs/status,verbs=get;update;patch

func (r *CatalogServiceCatalogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogservicecatalog", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogServiceCatalog")
...
func (r *CatalogServiceCatalogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.CatalogServiceCatalog{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.CatalogServiceCatalog{}.APIVersion, servicev1alpha1.CatalogServiceCatalog{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.CatalogServiceCatalog{}).
