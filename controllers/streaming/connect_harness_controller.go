...
// +kubebuilder:rbac:groups=streaming.oci.kubeform.com,resources=connectharnesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=streaming.oci.kubeform.com,resources=connectharnesses/status,verbs=get;update;patch

func (r *ConnectHarnessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectharness", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectHarness")
...
func (r *ConnectHarnessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &streamingv1alpha1.ConnectHarness{}); err != nil {
			klog.Error(err, "unable to set up auditor", streamingv1alpha1.ConnectHarness{}.APIVersion, streamingv1alpha1.ConnectHarness{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&streamingv1alpha1.ConnectHarness{}).
