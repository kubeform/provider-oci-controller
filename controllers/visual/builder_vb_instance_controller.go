...
// BuilderVbInstanceReconciler reconciles a BuilderVbInstance object
type BuilderVbInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=visual.oci.kubeform.com,resources=buildervbinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=visual.oci.kubeform.com,resources=buildervbinstances/status,verbs=get;update;patch

func (r *BuilderVbInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("buildervbinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BuilderVbInstance")
...
func (r *BuilderVbInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &visualv1alpha1.BuilderVbInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", visualv1alpha1.BuilderVbInstance{}.APIVersion, visualv1alpha1.BuilderVbInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&visualv1alpha1.BuilderVbInstance{}).
...
func (r *BuilderVbInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for BuilderVbInstance")
			return nil
		}
		...
		resourceList := &visualv1alpha1.BuilderVbInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BuilderVbInstance")
			return nil
		}
