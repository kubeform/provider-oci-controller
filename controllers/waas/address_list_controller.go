...
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=addresslists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=addresslists/status,verbs=get;update;patch

func (r *AddressListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("addresslist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AddressList")
...
func (r *AddressListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &waasv1alpha1.AddressList{}); err != nil {
			klog.Error(err, "unable to set up auditor", waasv1alpha1.AddressList{}.APIVersion, waasv1alpha1.AddressList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&waasv1alpha1.AddressList{}).
