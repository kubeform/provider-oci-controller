...
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=customprotectionrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=customprotectionrules/status,verbs=get;update;patch

func (r *CustomProtectionRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customprotectionrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomProtectionRule")
...
func (r *CustomProtectionRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &waasv1alpha1.CustomProtectionRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", waasv1alpha1.CustomProtectionRule{}.APIVersion, waasv1alpha1.CustomProtectionRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&waasv1alpha1.CustomProtectionRule{}).
