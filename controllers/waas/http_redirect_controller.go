...
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=httpredirects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=httpredirects/status,verbs=get;update;patch

func (r *HttpRedirectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("httpredirect", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HttpRedirect")
...
func (r *HttpRedirectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &waasv1alpha1.HttpRedirect{}); err != nil {
			klog.Error(err, "unable to set up auditor", waasv1alpha1.HttpRedirect{}.APIVersion, waasv1alpha1.HttpRedirect{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&waasv1alpha1.HttpRedirect{}).
