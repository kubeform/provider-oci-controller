...
// PurgeCacheReconciler reconciles a PurgeCache object
type PurgeCacheReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=purgecaches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=purgecaches/status,verbs=get;update;patch

func (r *PurgeCacheReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("purgecache", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PurgeCache")
...
func (r *PurgeCacheReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &waasv1alpha1.PurgeCache{}); err != nil {
			klog.Error(err, "unable to set up auditor", waasv1alpha1.PurgeCache{}.APIVersion, waasv1alpha1.PurgeCache{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&waasv1alpha1.PurgeCache{}).
