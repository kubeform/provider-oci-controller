...
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=waaspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waas.oci.kubeform.com,resources=waaspolicies/status,verbs=get;update;patch

func (r *WaasPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("waaspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WaasPolicy")
...
func (r *WaasPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &waasv1alpha1.WaasPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", waasv1alpha1.WaasPolicy{}.APIVersion, waasv1alpha1.WaasPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&waasv1alpha1.WaasPolicy{}).
