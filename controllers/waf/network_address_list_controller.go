...
// +kubebuilder:rbac:groups=waf.oci.kubeform.com,resources=networkaddresslists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waf.oci.kubeform.com,resources=networkaddresslists/status,verbs=get;update;patch

func (r *NetworkAddressListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkaddresslist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkAddressList")
...
func (r *NetworkAddressListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafv1alpha1.NetworkAddressList{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafv1alpha1.NetworkAddressList{}.APIVersion, wafv1alpha1.NetworkAddressList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafv1alpha1.NetworkAddressList{}).
