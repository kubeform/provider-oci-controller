...
// +kubebuilder:rbac:groups=waf.oci.kubeform.com,resources=webappfirewallpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waf.oci.kubeform.com,resources=webappfirewallpolicies/status,verbs=get;update;patch

func (r *WebAppFirewallPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("webappfirewallpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WebAppFirewallPolicy")
...
func (r *WebAppFirewallPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafv1alpha1.WebAppFirewallPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafv1alpha1.WebAppFirewallPolicy{}.APIVersion, wafv1alpha1.WebAppFirewallPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafv1alpha1.WebAppFirewallPolicy{}).
